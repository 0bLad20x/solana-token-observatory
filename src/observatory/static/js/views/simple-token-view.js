...
export class SimpleTokenView {
  ...
  render({ tokens, selectedMint }) {
    ...
    for (const token of this.#visibleTokens(tokens, selectedMint)) {
      const button = document.createElement("button");
      button.type = "button";
      button.style.textAlign = "left";
      button.style.padding = "10px";
      button.style.borderRadius = "8px";
      button.style.border = token.mint === selectedMint
        ? "1px solid #49D9FF"
        : "1px solid #252C3D";
      button.style.background = "#0F1420";
      button.style.color = "#F3F5F8";
      button.style.cursor = "pointer";
      button.setAttribute("aria-pressed", token.mint === selectedMint ? "true" : "false");
  ...
  #visibleTokens(tokens, selectedMint) {
    const active = tokens
