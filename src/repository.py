...
class MintRepository:
    ...
    def load_active_mints_by_priority(self, priority: int) -> list[str]:
        with self._pool.connection() as connection:
            rows = connection.execute(
                "SELECT mint FROM mints WHERE tracking_enabled = true AND priority = %s",
                (priority,),
            ).fetchall()
    ...
    def insert_new_mints(self, candidates: Sequence[str]) -> int:
        ...
        with self._pool.connection() as connection:
            with connection.cursor() as cursor:
                cursor.execute("SELECT mint FROM mints WHERE mint = ANY(%s)", (unique_candidates,))
                existing = {row[0] for row in cursor.fetchall()}
                new_candidates = [mint for mint in unique_candidates if mint not in existing]
                if new_candidates:
                    cursor.executemany(
                        "INSERT INTO mints (mint, priority, tracking_enabled) VALUES (%s, 1, true) ON CONFLICT (mint) DO NOTHING",
                        [(mint,) for mint in new_candidates],
                    )
            connection.commit()
    ...
    def store_tokens_grouped(self, tokens: Sequence[dict[str, Any]]) -> StoreSummary:
        ...
        with self._pool.connection() as connection:
            with connection.cursor() as cursor:
                cursor.execute("SELECT mint, name FROM mints WHERE mint = ANY(%s)", (mint_addresses,))
                existing_names = dict(cursor.fetchall())
                enriched = sum(1 for mint in mint_addresses if existing_names.get(mint) is None)
                cursor.executemany(
                    """
                    INSERT INTO mints (
