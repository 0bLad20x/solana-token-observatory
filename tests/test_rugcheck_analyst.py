...
class RugCheckAnalystTests(unittest.TestCase):
    def test_rugcheck_analysis_uses_one_strong_model_request_with_raw_evidence(self) -> None:
        ...
        self.assertIn("external provider", request["messages"][0]["content"])

        user_message = request["messages"][1]["content"]
        context_text = user_message.split("RugCheck external evidence JSON:\n", 1)[1]
        self.assertEqual(json.loads(context_text), evidence)
        ...
        self.assertGreater(result["evidence"]["rough_report_tokens"], 0)
